...
void UBTS_ChaseBounds::TickNode(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
	...
	// Get BehaviorManager for state changes (AC_AI_BehaviorManager Blueprint reparented to UAIBehaviorManagerComponent)
	...
	if (DistanceFromStart > ChaseDistance)
	{
		...
		if (BehaviorManager)
		{
			BehaviorManager->SetState(ESLFAIStates::Investigating);
