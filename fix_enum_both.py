# fix_enum_both.py
# Fixes enum pin values by searching for BOTH old and new enum names
#
# Run in Unreal Editor Python console:
#   exec(open(r"C:\scripts\SLFConversion\fix_enum_both.py").read())
...
def main():
    print("\n" + "="*70)
    print("  E_ValueType / ESLFValueType Enum Pin Fixer")
    print("  Searches for BOTH old and new enum names")
    print("="*70)
    ...
    for bp_path in BLUEPRINTS_TO_FIX:
        ...
        # Fix enum values - use NUMERIC values for C++ enum compatibility
        # NewEnumerator0 (CurrentValue) = 0
        # NewEnumerator1 (MaxValue) = 1

        # Search for E_ValueType pins (before redirect)
        fixed0 = fixer.fix_enum_pin_values(bp, "E_ValueType", "NewEnumerator0", "0")
        fixed1 = fixer.fix_enum_pin_values(bp, "E_ValueType", "NewEnumerator1", "1")

        if fixed0 > 0 or fixed1 > 0:
            print(f"      Fixed {fixed0} CurrentValue(0), {fixed1} MaxValue(1)")
