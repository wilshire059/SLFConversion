# migrate_b_interactable_only.py
# Migrate ONLY B_Interactable first, so it's saved before B_Container loads

import unreal

BP_PATH = "/Game/SoulslikeFramework/Blueprints/_WorldActors/Interactables/B_Interactable"
CPP_PATH = "/Script/SLFConversion.SLFInteractableBase"


def main():
    unreal.log_warning("=" * 60)
    unreal.log_warning("MIGRATING B_INTERACTABLE ONLY")
    unreal.log_warning("=" * 60)
    ...
    unreal.log_warning("=" * 60)
    unreal.log_warning("B_INTERACTABLE MIGRATION COMPLETE")
    unreal.log_warning("=" * 60)
