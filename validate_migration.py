    ...
    if 'Functions' in bp_data:
        ...
        if isinstance(funcs, list):
            ...
            for func in funcs['List']:
                if isinstance(func, dict) and 'Name' in func:
                    functions.append(func['Name'])
    # Check FunctionSignatures.Functions (BlueprintDNA format)
    if 'FunctionSignatures' in bp_data and isinstance(bp_data['FunctionSignatures'], dict):
        if 'Functions' in bp_data['FunctionSignatures']:
            funcs_list = bp_data['FunctionSignatures']['Functions']
            if isinstance(funcs_list, list):
                for func in funcs_list:
                    if isinstance(func, dict) and 'Name' in func:
                        func_name = func['Name']
                        if func_name not in ['EventGraph', 'ConstructionScript']:
                            functions.append(func_name)
            elif isinstance(funcs_list, dict):
                for func_name in funcs_list.keys():
                    if func_name not in ['EventGraph', 'ConstructionScript']:
                        functions.append(func_name)
    # Check Logic.AllGraphs for function graph names
    if 'Logic' in bp_data and isinstance(bp_data['Logic'], dict):
        if 'AllGraphs' in bp_data['Logic']:
            all_graphs = bp_data['Logic']['AllGraphs']
            if isinstance(all_graphs, list):
                for graph in all_graphs:
                    if isinstance(graph, dict) and 'GraphName' in graph:
                        graph_name = graph['GraphName']
                        if graph_name not in ['EventGraph', 'ConstructionScript']:
                            functions.append(graph_name)
            elif isinstance(all_graphs, dict):
                for graph_name in all_graphs.keys():
                    if graph_name not in ['EventGraph', 'ConstructionScript']:
                        functions.append(graph_name)
    # Check top-level Graphs
...
def find_in_cpp(name, cpp_content):
    ...
    if re.search(pattern, cpp_content, re.IGNORECASE):
        return True
    # Also try without "Event" prefix (Blueprint "Event Foo" -> C++ "Foo")
    if normalized.startswith('Event'):
        without_event = normalized[5:]
        if without_event:
            pattern2 = r'\b' + re.escape(without_event) + r'\b'
            if re.search(pattern2, cpp_content, re.IGNORECASE):
                return True
    # Also try with "Event" prefix added (C++ uses EventFoo naming)
    pattern3 = r'\bEvent' + re.escape(normalized) + r'\b'
    if re.search(pattern3, cpp_content, re.IGNORECASE):
        return True
    return False
